...
$.extend($.ui, { datetimepicker: { version: "1.0.0"} });
    ...
    $.extend(Datetimepicker.prototype, {
        ...
        _doKeyDown: function(event) {
            ...
            if ($.datetimepicker._datepickerShowing)
                ...
                case 13: var sel = $('td.' + $.datetimepicker._dayOverClass, inst.dpDiv).
                    ...
                    if (sel[0])
                        $.datetimepicker._selectDay(event.target, inst.selectedMonth, inst.selectedYear, sel[0], inst.currentHour, inst.currentMinute, inst.currentSecond);  ////$.datetimepicker._selectDay(event.target, inst.selectedMonth, inst.selectedYear, sel[0], inst.currentHour, inst.currentMinute, inst.currentAMPM);
                    else
                        $.datetimepicker._hideDatepicker();
        ...
        _updateDatepicker: function(inst) {
            ...
            $("#DP_jQuery_Second_" + dpuuid).val(inst.selectedSecond);            
       ...
       // _selectDay: function(id, month, year, td, hh, mm, am) {
        _selectDay: function(id, month, year, td, hh, mm, ss) {
            var target = $(id);
        ...
        */
        ...
        _generateHTML: function(inst) {
            ...
            for (var row = 0; row < numMonths[0]; row++) {
                ...
                // Second Drop Down
                html += '&nbsp;: <select id="DP_jQuery_Second_' + dpuuid + '">';
                for (i = 0; i < 60; i++) {
        ...
        _formatDate: function(inst, day, month, year) {
           ...
           // var DateString = '' + inst.currentMonth + '/' + inst.selectedDay + '/' + inst.selectedYear + ' ' + Hour + ':' + MinuteString + ' ' + SecondString;
            ...
            var MonthString = String(inst.currentMonth);
            
            if (MonthString.length == 1)
                MonthString = "0" + MonthString;
            
            var DayString = inst.selectedDay;
            if (DayString.length == 1)
                DayString = "0" + DayString;
            
            var DateString = '' + inst.selectedYear  + '-' + MonthString + '-' + DayString + ' ' + Hour + ':' + MinuteString + ':' + SecondString;
